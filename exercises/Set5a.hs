...
totalPrice (MkShoppingEntry _ pric qtty) = pric * (fromIntegral qtty)
...
buyOneMore (MkShoppingEntry name pric qtty) = MkShoppingEntry name pric (qtty+1)
...
tick :: UpDown -> UpDown
tick (UpMode n) = UpMode (n+1)
tick (DownMode n) = DownMode (n-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpMode n) = DownMode n
toggle (DownMode n) = UpMode n
...
rgb Green = [0, 1, 0]
rgb Blue  = [0, 0, 1]
rgb (Mix c1 c2) = zipWith (\ x y -> (x+y)/2) c1' c2'
    where c1' = rgb c1
          c2' = rgb c2
rgb (Invert c) = map (1-) c'
    where c' = rgb c
