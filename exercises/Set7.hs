...
reverseNonEmpty (x :| xs) = y :| ys
  where
    (y : ys) = reverse xs ++ [x] -- work on normal lists and then construct a NonEmpty
...
instance Semigroup Velocity where
  -- should use the harmonic mean, but MOOC authors didn't seem to think of it.
...
-- We need Ord because our requirements of "uniqueness" and "ascending order"
-- requires equality & comparability, hence the Ord constraint
-- Also, I used `sort` and `union` instead of implementing my own functions.
instance Ord a => Semigroup (Set a) where
  (Set a) <> (Set b) = Set (sort $ a `union` b)
...
showOp :: Int -> Int -> String -> String
showOp i j op = show i ++ op ++ show j

show1 :: Operation1 -> String
show1 (Add1 i j) = showOp i j "+"
show1 (Subtract1 i j) = showOp i j "-"
show1 (Multiply1 i j) = showOp i j "*"
...
instance Operation2 Add2 where
  ...
  show2 (Add2 i j) = showOp i j "+"

instance Operation2 Subtract2 where
  compute2 (Subtract2 i j) = i - j
  show2 (Subtract2 i j) = showOp i j "-"

instance Operation2 Multiply2 where
  compute2 (Multiply2 i j) = i * j
  show2 (Multiply2 i j) = showOp i j "*"
...
passwordAllowed pw (ContainsSome conts) = any (`elem` conts) pw
passwordAllowed pw (DoesNotContain conts) = not (any (`elem` conts) pw)
...
evaluate (Operation "-" l r) = evaluate l - evaluate r
evaluate (Operation "*" l r) = evaluate l * evaluate r
evaluate (Operation "/" l r) = evaluate l `div` evaluate r
evaluate Operation {} = 0
