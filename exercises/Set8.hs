...
blendColor (Color r g b) (Color r' g' b') = Color newr newg newb
  where
    mean2 x y = div (x + y) 2
    newr = mean2 r r'
    newg = mean2 g g'
...
rectangle x0 y0 w h = Shape f
  where
    f (Coord x y) = inbounds x x0 w && inbounds y y0 h
    inbounds z z0 p = z >= z0 && z < z0 + p 
...
cut (Shape target) (Shape cutter) = Shape f
  where f coord = target coord && not (cutter coord)
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where
      g (Coord x y) = blendNColor (f (Coord x y))
                                  (f (Coord (x+1) y))
                                  (f (Coord (x-1) y))
                                  (f (Coord x (y+1)))
                                  (f (Coord x (y-1)))
      blendNColor (Color r0 g0 b0)
                  (Color r1 g1 b1)
                  (Color r2 g2 b2)
                  (Color r3 g3 b3)
                  (Color r4 g4 b4) = Color (div (r0 + r1 + r2 + r3 + r4) 5)
